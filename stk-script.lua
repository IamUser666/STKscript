...
-- Определение режима лобби
local function checkLobbyMode()
    -- Проверяем наличие объектов, характерных для лобби
    if Workspace:FindFirstChild("LobbyArea") then
        lobbyMode = true
    elseif Workspace:FindFirstChild("GameArea") then
        lobbyMode = false
    else
        -- Альтернативный способ: проверка по времени игры
        lobbyMode = (game:GetService("Workspace"):FindFirstChild("RoundTimer") == nil)
    end
end
...
local function createESP(player)
    ...
    removeESP(player) -- Удаляем старый ESP если есть
...
local function updateESP()
    checkLobbyMode() -- Обновляем статус лобби
        ...
        if player.Character and Player.Character then
            ...
            if root and myRoot then
                ...
                if lobbyMode then
                    ...
                    data.Highlight.FillColor = Color3.new(0.5, 0.5, 0.5) -- Серый цвет
                    data.Highlight.OutlineColor = Color3.new(0.7, 0.7, 0.7) -- Светло-серый
                    
                    data.DistanceLabel.Visible = true
                    data.DistanceLabel.Text = string.format("%.1fm", distance)
                    data.DistanceLabel.TextColor3 = Color3.new(0.8, 0.8, 0.8) -- Серый текст
                else
                    -- Игровой режим
...
-- =====================================
-- Power: Clones System (Упрощенная и стабильная версия)
-- =====================================
local cloneUI = nil
...
-- =====================================
-- Power: Free Camera System (Рабочая версия)
-- =====================================
...
local function createCameraControls()
    ...
    cameraControls.Size = UDim2.new(0, 300, 0, 300)
    cameraControls.Position = UDim2.new(0.5, -150, 0.5, -150)
    ...
    stopButton.Position = UDim2.new(1, -45, 0, 5)
    ...
    cameraControls.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement then
            if input.UserInputState == Enum.UserInputState.Change then
                ...
                cameraRotX = cameraRotX - delta.X * rotationSpeed / 100
                cameraRotY = cameraRotY - delta.Y * rotationSpeed / 100
                cameraRotY = math.clamp(cameraRotY, -math.rad(80), math.rad(80))
            end
        end
    end)
    
    -- Обработка движения
    local function handleMovement(key, state)
        if key == Enum.KeyCode.W then
            moveVector = Vector3.new(moveVector.X, moveVector.Y, state and -1 or 0)
        elseif key == Enum.KeyCode.S then
            moveVector = Vector3.new(moveVector.X, moveVector.Y, state and 1 or 0)
        elseif key == Enum.KeyCode.A then
            moveVector = Vector3.new(state and -1 or 0, moveVector.Y, moveVector.Z)
        elseif key == Enum.KeyCode.D then
            moveVector = Vector3.new(state and 1 or 0, moveVector.Y, moveVector.Z)
        elseif key == Enum.KeyCode.E then
            moveVector = Vector3.new(moveVector.X, state and 1 or 0, moveVector.Z)
    ...
    UserInputService.InputBegan:Connect(function(input)
        if input.KeyCode then
            handleMovement(input.KeyCode, true)
        end
    end)
    
    UserInputService.InputEnded:Connect(function(input)
        if input.KeyCode then
            handleMovement(input.KeyCode, false)
        end
    end)
...
local function activateFreeCamera()
    ...
    cameraOrigin = workspace.CurrentCamera.CFrame
    cameraRotX = 0
    cameraRotY = 0
    ...
    cameraConnection = RunService.Heartbeat:Connect(function(deltaTime)
        if freeCameraActive and cameraOrigin then
            -- Вычисляем направление движения с учетом поворота
            local moveDirection = Vector3.new(
                moveVector.X * math.cos(cameraRotX) - moveVector.Z * math.sin(cameraRotX),
                moveVector.Y,
                moveVector.X * math.sin(cameraRotX) + moveVector.Z * math.cos(cameraRotX)
            )
            
            -- Применяем движение
            cameraOrigin = cameraOrigin + (moveDirection * moveSpeed * deltaTime)
            
            -- Применяем вращение
            local rotCFrame = CFrame.Angles(0, cameraRotX, 0) * CFrame.Angles(cameraRotY, 0, 0)
            local newCFrame = cameraOrigin * rotCFrame
...
-- Автообновление ESP между раундами
local function reconnectESP()
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= Player and not trackedPlayers[player] then
            if player.Character then
                createESP(player)
            else
                player.CharacterAdded:Once(function()
