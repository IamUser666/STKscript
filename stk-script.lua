...
MenuButton.Position = UDim2.new(0, 10, 0.5, -15)
...
local function updateMenuPosition()
    ...
    local buttonPos = MenuButton.AbsolutePosition
    MenuFrame.Position = UDim2.new(0, buttonPos.X + 60, 0, buttonPos.Y)
end
...
local function isKiller(player)
    -- Логика определения киллера (может отличаться в разных плейсах)
    return player:FindFirstChild("Knife") or player:FindFirstChild("KillerTag")
end
...
local function removeESP(player)
    if trackedPlayers[player] then
        ...
        trackedPlayers[player].DistanceLabel:Destroy()
...
-- =====================================
-- Power: Clones System (Обновлённая версия)
-- =====================================
local cloneUI = nil
    ...
    UserInputService.TouchMoved:Connect(function(input, processed)
        if draggingCamera and not processed then
            ...
            cameraControls.Position = UDim2.new(
                uiStartPos.X.Scale, uiStartPos.X.Offset + delta.X,
                uiStartPos.Y.Scale, uiStartPos.Y.Scale + delta.Y
            )
        end
    end)
